...
for i = 1:length(traces)
    ...
    trace_data.user = trace_name_parts{1};
    trace_data.date = trace_name_parts{2};
    trace_data.number = trace_name_parts{4}; 
    ...
    for j = 1:length(task_plot_list)
        ...
        %Plot the torque from force using lighter colors.
        plot(tasks{task}.data.Time_s_ - time0, movmean(tasks{task}.data.TorqueFromForce_pNnm_, movemean_step), 'Linewidth', linewidth, 'Color', lighten_plot_color(default_MATLAB_colors(j),0.5));
    end
	plot_xlines(trace_data.fork_position.flip_times - time0);
...
function stall_lines_plot(stall_start, time0, stall_window, indir_stall_torque_XXs, dir_stall_torque_XXs)
    ...
    text(stall_start - time0, 30, ['Stall torque from force within 60s:    ' num2str(indir_stall_torque_XXs)], 'FontSize', 14);
